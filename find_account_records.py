...
for i, r in enumerate(doc.records):
    if r['tag'] in (2201, 2202):
        ...
        if any(k in s.lower() for k in ['nomor rekening', 'account number', 'mata uang', 'tabungan']):
            print(f"Label at Rec {i:05d} Tag {r['tag']:04d}: {repr(s)}")
            # Show next 20 text records
            for k in range(i + 1, min(len(doc.records), i + 30)):
                if doc.records[k]['tag'] in (2201, 2202):
                    ...
                    if any(c.isdigit() for c in s2) and len(s2) >= 8:
                        break
